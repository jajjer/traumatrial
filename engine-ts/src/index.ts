// Public entry point for the traumatrial-match TS engine.
// Mirrors engine/traumatrial_match/match.py — same operators, same trace
// format, same confidence rubric. Pure functions, zero dependencies.
...
export function match(patient: Patient, trial: Trial): MatchResult {
  ...
  for (const rule of trial.inclusion) {
  ...
  for (const rule of trial.exclusion) {
...
/** Evaluate one patient against many trials. Eligible-first, then confidence desc. */
